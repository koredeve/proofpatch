  ...
  return (
    <div className="fade-up">
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        {TABS.map(([k,l])=>(
          <button key={k} onClick={()=>setTab(k)} className={`rounded-lg border px-4 py-2 text-sm transition-colors ${tab===k?"border-manila text-manila":"border-[#232833] text-fog hover:border-fog"}`}>{l}</button>
        ))}
      </div>
      <div className="card mt-6 divide-y divide-[#232833]">
        ...
        {rows.map((r:any,i:number)=>(
          <Link key={r.id} href={`/profile/${r.username}`} className="flex items-center gap-4 px-5 py-3.5 hover:bg-white/[.03] transition-colors">
            <span className={`w-7 text-center font-mono font-bold ${i===0?"text-manila":i===1?"text-[#c0c8d4]":i===2?"text-[#cd7f32]":"text-fog"}`}>{i+1}</span>
