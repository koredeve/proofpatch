...
function simulateVerdict(claim, rules, contexts) {
  ...
  for (const { ev, web } of contexts) {
    ...
    const retrievalFailed = web.startsWith('[retrieval failed');
    let score;
    if (retrievalFailed) {
      // Degraded mode: judge on the researcher's quoted passage alone, minus a
      // 10% transparency penalty since live corroboration was impossible.
      score = direct * w * 0.9;
    } else {
      // Base signal: strongest of quoted-passage match / live-page match.
      // Corroboration bonus: quote AND independently retrieved page agreeing.
      const base = Math.max(direct, Math.min(onPage * 0.75, 1));
      const corroboration = Math.min(direct, onPage);
      score = Math.min(base + 0.15 * corroboration, 1) * w;
    }
    details.push({ url: ev.url, direct: +direct.toFixed(2), onPage: +onPage.toFixed(2),
                   weight: w, retrievalFailed, passageNegates,
                   ...(retrievalFailed ? { note: web.slice(0, 120) } : {}) });
    if (passageNegates) {
